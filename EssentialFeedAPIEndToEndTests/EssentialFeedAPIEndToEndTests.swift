...
//
//  Created by Andy Hao on 2022/10/11.
//
...
class EssentialFeedAPIEndToEndTests: XCTestCase {
    func test_endToEndTestServerGETFeedResult_mahchesFixedTestAccountData() {
        ...
        var reveivedResult:LoadFeedResult?
        loader.load { result  in
            reveivedResult = result
            exp.fulfill()
        }

        wait(for: [exp], timeout: 20.0)
        
        switch reveivedResult {
        ...
        case let .failure(error)?:
            XCTFail("Expexted successful feed result, got \(error) instead")
        default:
            XCTFail("Expexted successful feed result, got no result instead")
        }
    }
